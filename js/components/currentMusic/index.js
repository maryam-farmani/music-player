...
const repeatStyle =document.querySelector(".repeat-style");
const shuffleMusic = document.querySelector("#shuffle-music");
const shuffleStyle = document.querySelector(".shuffle-style");
const volumeMusic = document.querySelector("#volume-music");

let indexOfCurrentMusic = 0;
let shuffleMusicOn= false;
...
shuffleMusic.addEventListener("click" , ()=>{
  shuffleMusicOn =! shuffleMusicOn;
  if (shuffleMusicOn) {
    shuffleStyle.style.fill = "blue";
  
  } else {
    shuffleStyle.style.fill = "#4F4F4F";
  }
});

volumeMusic.onchange = function () {
  audio.volume = volumeMusic.value/10 ;  
};

repeatMusic.addEventListener("click", ()=>{
  repeatMusicState =! repeatMusicState;
  if (repeatMusicState) {
    repeatStyle.style.fill = "blue";
  }else {
    repeatStyle.style.fill = "#4F4F4F";
  }
});

function handleNextMusic() {
  if(shuffleMusicOn)
    return updateCurrentMusicUI(musics()[Math.floor(Math.random()* musics().length)]);
  if(repeatMusicState) 
    ...
    [...musicsContainer.children].forEach((musicElement)=>{
      musicElement.addEventListener("click", ()=>{
        indexOfCurrentMusic = musics().findIndex(
          (item) => item.id === parseInt(musicElement.dataset.id)
        );
